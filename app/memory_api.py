...
class Settings(BaseSettings):
    ...
    API_ACCESS_KEY: str | None = None  # Required in prod
...
async def log_scrubber(request: Request, call_next):
    ...
    try:
        ...
        # Minimal log: method path status time. No headers, no body, no secrets.
        try:
            status = getattr(request.state, "response_status", None)
        except Exception:
            status = None
        status_code = status or getattr(getattr(request, "response", None), "status_code", None) or "-"
        print(f"{request.method} {request.url.path} -> {status_code} ({dt_ms}ms)")
        # Note: Render logs stdout; we keep it minimal to avoid leaking secrets.
...
def _rate_key(request: Request) -> str:
    ...
    return api_key or client_ip  # prefer API key; fallback to IP

def _limit_for_path(path: str) -> int:
    if path == "/context/build":
        return max(1, settings.RATE_LIMIT_CONTEXT_PER_MIN)
    return max(1, settings.RATE_LIMIT_DEFAULT_PER_MIN)

@app.middleware("http")
async def rate_limiter(request: Request, call_next):
    # Skip open paths and if no key configured (dev)
    open_paths = {"/healthz", "/openapi.json", "/docs", "/docs/oauth2-redirect", "/redoc"}
    if request.url.path in open_paths:
        return await call_next(request)

    now = int(time.time())
    bucket = _rate_store.setdefault(_rate_key(request), {})
    ...
    if entry["count"] >= limit:
        retry_in = WINDOW_SECONDS - (now - entry["window_start"])
        return JSONResponse(
            status_code=429,
            content={"detail": "Rate limit exceeded", "limit_per_min": limit, "retry_in_seconds": retry_in},
        )

    entry["count"] += 1
    resp = await call_next(request)
    request.state.response_status = resp.status_code
    return resp
...
@app.get("/healthz")
def healthz():
    return {"status": "ok", "embedding_model": settings.EMBEDDING_MODEL, "gpt_model": settings.GPT_MODEL}
...
def save_memory(item: MemoryItem):
    ...
    collections[item.role].add(
        ...
        metadatas=[{"user_id": item.user_id, "tags": ",".join(item.tags), "created_at": datetime.utcnow().isoformat()}],
        ids=[mem_id]
    )
...
def build_context(item: ContextBuildRequest):
    def vector_search(role_key: str) -> list[dict[str, Any]]:
        if role_key not in collections: return []
        ...
        matches = []
        for doc, md, dist in zip(results.get("documents", [[]])[0],
    ...
    context_text = "\n\n".join(f"{m['content']}" for m in top_ranked)
